...
void vec_and_bloom() {
    BloomFilter<uintptr_t> bf(39260, 6);
    std::vector<DatasetEntry> set;
    set.reserve(1024);
    for(int i = 0; i < 1024; i++){
        auto addr = &arr[random_indices[i]];//reinterpret_cast<uintptr_t>(&arr[random_indices[i]]);
        // まずbloom filterに通す
        if(!bf.Contains(reinterpret_cast<uintptr_t>(addr), sizeof(uintptr_t))){
            //確実に入っていないなら挿入
            bf.Add(reinterpret_cast<uintptr_t>(addr), sizeof(uintptr_t));
            set.emplace_back(addr, random_indices[i]);
            continue;
        } 
        // 入っていそうなら詳細検索
